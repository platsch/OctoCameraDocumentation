$(function() {
    function OctoCamDoxSettingsViewModel(parameters) {
        ...
        self.headCameraOffset = function() {
            ...
            var x = self.objectPositionX() - parseFloat(self.settings.plugins.OctoCamDox.camera.head.x());
            var y = self.objectPositionY() - parseFloat(self.settings.plugins.OctoCamDox.camera.head.y());
            self.control.sendCustomCommand({command: "G1 X" + x + " Y" + y + " Z" + self.settings.plugins.OctoCamDox.camera.head.z() + " F3000"});
        ...
        self.saveHeadCameraOffset = function() {
            //save values...
            self.settings.plugins.OctoCamDox.camera.head.x(parseFloat(self.settings.plugins.OctoCamDox.camera.head.x())-self.offsetCorrectionX());
            self.settings.plugins.OctoCamDox.camera.head.y(parseFloat(self.settings.plugins.OctoCamDox.camera.head.y())-self.offsetCorrectionY());
        ...
        self.bedCameraPosition = function() {
            ...
            //move camera to object
            var x = parseFloat(self.settings.plugins.OctoCamDox.camera.bed.x());
            var y = parseFloat(self.settings.plugins.OctoCamDox.camera.bed.y());
            self.control.sendCustomCommand({command: "G1 X" + x + " Y" + y + " Z" + self.settings.plugins.OctoCamDox.camera.bed.z() + " F3000"});
        ...
        self.saveBedCameraPosition = function() {
            //save values
            self.settings.plugins.OctoCamDox.camera.bed.x(parseFloat(self.settings.plugins.OctoCamDox.camera.bed.x())+self.offsetCorrectionX());
            self.settings.plugins.OctoCamDox.camera.bed.y(parseFloat(self.settings.plugins.OctoCamDox.camera.bed.y())+self.offsetCorrectionY());
        ...
        self.pnpNozzleOffset = function() {
            ...
            self.control.sendCustomCommand({command: "G1 X100 Y150 F3000"});
            // Switch to VacNozzle extruder
            self.control.sendCustomCommand({command: "T" + self.settings.plugins.OctoCamDox.vacnozzle.extruder_nr().toString()});

            //move camera to object
            var x = parseFloat(self.settings.plugins.OctoCamDox.camera.bed.x()) - parseFloat(self.settings.plugins.OctoCamDox.vacnozzle.x());
            var y = parseFloat(self.settings.plugins.OctoCamDox.camera.bed.y()) - parseFloat(self.settings.plugins.OctoCamDox.vacnozzle.y());
            self.control.sendCustomCommand({command: "G1 X" + x + " Y" + y + " Z" + self.settings.plugins.OctoCamDox.camera.bed.z() + " F3000"});
        ...
        self.savePnpNozzleOffset = function() {
            //save values
            self.settings.plugins.OctoCamDox.vacnozzle.x(parseFloat(self.settings.plugins.OctoCamDox.vacnozzle.x())-self.offsetCorrectionX());
            self.settings.plugins.OctoCamDox.vacnozzle.y(parseFloat(self.settings.plugins.OctoCamDox.vacnozzle.y())-self.offsetCorrectionY());
        ...
        self.trayPosition = function(corner) {
            ...
            switch (corner) {
                case "TL":
                    var rows = parseFloat(self.settings.plugins.OctoCamDox.tray.rows());
                    cornerOffsetY = rows*parseFloat(self.settings.plugins.OctoCamDox.tray.boxsize()) + (rows+1)*parseFloat(self.settings.plugins.OctoCamDox.tray.rimsize());
                    self.statusTrayPosition(false);
                    break;
                case "TR":
                    var rows = parseFloat(self.settings.plugins.OctoCamDox.tray.rows());
                    var cols = parseFloat(self.settings.plugins.OctoCamDox.tray.columns());
                    cornerOffsetY = rows*parseFloat(self.settings.plugins.OctoCamDox.tray.boxsize()) + (rows+1)*parseFloat(self.settings.plugins.OctoCamDox.tray.rimsize());
                    cornerOffsetX = cols*parseFloat(self.settings.plugins.OctoCamDox.tray.boxsize()) + (cols+1)*parseFloat(self.settings.plugins.OctoCamDox.tray.rimsize());
                    self.statusTrayPosition(false);
                    break;
                case "BR":
                    var cols = parseFloat(self.settings.plugins.OctoCamDox.tray.columns());
            ...
            //move camera to tray
            var x = parseFloat(self.settings.plugins.OctoCamDox.tray.x()) + cornerOffsetX - parseFloat(self.settings.plugins.OctoCamDox.camera.head.x());
            var y = parseFloat(self.settings.plugins.OctoCamDox.tray.y()) + cornerOffsetY - parseFloat(self.settings.plugins.OctoCamDox.camera.head.y());
            var z = parseFloat(self.settings.plugins.OctoCamDox.tray.z()) + parseFloat(self.settings.plugins.OctoCamDox.camera.head.z());
            console.log(self.settings.plugins.OctoCamDox.tray.z());
            console.log(self.settings.plugins.OctoCamDox.camera.head.z());
            console.log(z);
        ...
        self.saveTrayPosition = function() {
            //save values
            self.settings.plugins.OctoCamDox.tray.x(parseFloat(self.settings.plugins.OctoCamDox.tray.x())+self.offsetCorrectionX());
            self.settings.plugins.OctoCamDox.tray.y(parseFloat(self.settings.plugins.OctoCamDox.tray.y())+self.offsetCorrectionY());
        ...
        self._getImage = function(imagetype, callback) {
            $.ajax({
                url: PLUGIN_BASEURL + "OctoCamDox/camera_image?imagetype=" + imagetype,
                type: "GET",
                dataType: "json",
